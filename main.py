...
def fetch_history(token):
  ...
  try:
    ...
    res = KITE_SESSION.get(url, params=params, timeout=5)
    DIAGNOSTICS["last_rest_ping"] = datetime.datetime.now(IST).strftime(
        "%H:%M:%S"
    )
...
def fire_order(
  ...
  if requested_type == "PROTECTED" or (
    ...
    if ltp > 0:
      ...
      log_event(
          f"REJECT GUARD ({sym}): Cannot place MARKET on MCX Option with 0"
          " LTP."
      )
      return
  ...
  try:
    res = KITE_SESSION.post(
        "https://kite.zerodha.com/oms/orders/regular", data=payload, timeout=5
    )
    if res.status_code == 200 and res.json().get("status") == "success":
      log_event(
          f"CONFIRMED ({sym}): ID {res.json().get('data', {}).get('order_id', 'N/A')}"
      )
    else:
